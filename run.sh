...
# This uses the ENV variables created by linking or predefined variables if the first ones are not availble
...
# Retry connecting to the database 10 times, every 5 seconds
for ((i=0;i<10;i++)); do
...
# Once a connection is established, see if we need to create a new database
if [[ $DB_CONNECTABLE -eq 0 ]]; then
...
# Let us know the database has already been created
